...
#Get prop_coeff value here
read -p 'Amount of multiplier in Nt [positive real number]: ' prop_coeff
...
$fdtd_bin $input_dir $boundary_cond $excitation_method $custom_name $output_file_type $output_dir $algorithm $num_subdomains $multithreading $overlap_size $comprehensive $prop_coeff $user_name $sim_description |& tee $output_dir/$log_file
